...
#include "lslidar_c16_decoder/lslidar_rawdata.hpp"
...
namespace lslidar_rawdata {
    ...
    void RawData::loadConfigFile(rclcpp::Node::SharedPtr node, rclcpp::Node::SharedPtr private_nh) {
        std::string model;
        std::string resolution_param;

        // private_nh->declare_parameter("start_angle", start_angle_, float(0));
        // private_nh->declare_parameter("end_angle", end_angle_, float(360));
        // private_nh->declare_parameter("distance_unit", distance_unit_, float(0.25));
        // private_nh->declare_parameter("calibration_file", calibration_file_, std::string(""));
        // private_nh->declare_parameter("scan_start_angle", scan_start_angle_, float(0.0));
        // private_nh->declare_parameter("scan_end_angle", scan_end_angle_, float(36000.0));
        // private_nh->declare_parameter("rpm", rpm_, 300);
        private_nh->declare_parameter("start_angle", start_angle_);
        private_nh->declare_parameter("end_angle", end_angle_);
        private_nh->declare_parameter("distance_unit", distance_unit_);
        private_nh->declare_parameter("calibration_file", calibration_file_);
        private_nh->declare_parameter("scan_start_angle", scan_start_angle_);
        ...
        if (start_angle_ < 0 || start_angle_ > 360 || end_angle_ < 0 || end_angle_ > 360) {
            start_angle_ = 0;
            end_angle_ = 360;
            RCLCPP_INFO(rclcpp::get_logger("rawdata"), "start angle and end angle select feature deactivated.");
        } else {
            RCLCPP_INFO(rclcpp::get_logger("rawdata"),"start angle and end angle select feature activated.");
        }

        angle_flag_ = true;
        if (start_angle_ > end_angle_) {
            angle_flag_ = false;
            RCLCPP_INFO(rclcpp::get_logger("rawdata"),"Start angle is smaller than end angle, not the normal state!");
        }
        RCLCPP_INFO(rclcpp::get_logger("rawdata"),"start_angle: %f, end_angle: %f, angle_flag: %d", start_angle_, end_angle_, angle_flag_);


        start_angle_ = start_angle_ / 180 * M_PI;
        end_angle_ = end_angle_ / 180 * M_PI;

        private_nh->declare_parameter("max_range", max_distance_);
        private_nh->declare_parameter("min_range", min_distance_);
        private_nh->declare_parameter("return_mode", return_mode_);
        private_nh->declare_parameter("config_vert", config_vert_);
        private_nh->declare_parameter("print_vert", print_vert_);
        private_nh->declare_parameter("config_vert_file", config_vert_file_);
        private_nh->declare_parameter("degree_mode", degree_mode_);
        ...
        if (2 == degree_mode_) {
            RCLCPP_INFO(rclcpp::get_logger("rawdata"),"vertical angle resolution: 2 degree");
        } else if (1 == degree_mode_) {
            RCLCPP_INFO(rclcpp::get_logger("rawdata"),"vertical angle resolution: 1.33 degree");
        }
        ...
        difop_sub_ = node->create_subscription<lslidar_c16_msg::msg::LslidarC16Packet>("lslidar_packet_difop_c16", 10, std::bind(&RawData::processDifop, this, std::placeholders::_1));
        // difop_sub_ = node.subscribe("lslidar_packet_difop_c16", 10, &RawData::processDifop, (RawData *) this);
    }

    void RawData::processDifop(const lslidar_c16_msg::msg::LslidarC16Packet &difop_msg) {
        ...
        if (config_vert_) {
            if (2 == version_data) {
                for (int i = 0; i < 16; i++) {
                    ...
                    RCLCPP_INFO(rclcpp::get_logger("rawdata"),"data vert: %d.",vert_angle);
        ...
        if ((data[8] == 0x04) && (data[9] == 0xB0)) {
            ...
            rpm_ = 300;
            RCLCPP_INFO(rclcpp::get_logger("rawdata"),"Invalid motor rpm!");
        }
        if (print_vert_) {
            RCLCPP_INFO(rclcpp::get_logger("rawdata"),"rpm is %d", rpm_);
        }
    }
 ...
 */
    void
    RawData::unpack(const lslidar_c16_msg::msg::LslidarC16Packet &pkt, VPointCloud::Ptr pointcloud,
                    int Packet_num,
        ...
        if (config_vert_angle) {
            for (int i = 0; i < LSC16_SCANS_PER_FIRING; i++) {
                ...
                if (print_vert_) {
                    RCLCPP_INFO(rclcpp::get_logger("rawdata"), "Channel %d Data  %f", i, scan_altitude[i] * RAD_TO_DEG);
                }
            }
            config_vert_angle = false;
        }
            ...
            for (int firing = 0, k = 0; firing < LSC16_FIRINGS_PER_BLOCK; firing++)  // 2
            {
                for (int dsr = 0; dsr < LSC16_SCANS_PER_FIRING; dsr++, k += RAW_SCAN_SIZE)  // 16   3
                {
                    ...
                    if ((distance2 > max_distance_) || (distance2 < min_distance_) ||
                        ...
                        sweep_data->scans[dsr].points.push_back(lslidar_c16_msg::msg::LslidarC16Point());
                        lslidar_c16_msg::msg::LslidarC16Point &new_point = sweep_data->scans[dsr].points[
