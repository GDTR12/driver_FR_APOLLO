#pragma once
#ifndef ___LSLIDAR_RAWDATA_HPP__
#define ___LSLIDAR_RAWDATA_HPP__


#include "stdio.h"
#include "stdint.h"
#include <pcl/point_types.h>  // 包含PCL点云类型的头文件
#include <pcl/point_cloud.h>  // 包含PCL点云对象的头文件
...
// Pre-compute the sine and cosine for the altitude angles.
    ...
    extern double scan_altitude[16];
 ...
 */
    ...
    // typedef pcl::PointCloud<::PointXYZIRT> VPointCloud;
...
/** \brief lslidar data conversion class */
    ...
    public:
        ...
        void loadConfigFile(rclcpp::Node::SharedPtr node, rclcpp::Node::SharedPtr private_nh);
        /*unpack the UDP packet and opuput PCL PointXYZI type*/
        void unpack(const lslidar_c16_msg::msg::LslidarC16Packet &pkt, VPointCloud::Ptr pointcloud,
                    int Packet_num,
                    lslidar_c16_msg::msg::LslidarC16Sweep::SharedPtr &sweep_data);
        
        void processDifop(const lslidar_c16_msg::msg::LslidarC16Packet &difop_msg);

        rclcpp::Subscription<lslidar_c16_msg::msg::LslidarC16Packet>::SharedPtr difop_sub_;
    ...
    extern float sin_azimuth_table[ROTATION_MAX_UNITS];
    extern float cos_azimuth_table[ROTATION_MAX_UNITS];


    extern float VERT_ANGLE[32];
    extern float HORI_ANGLE[32];
    extern float aIntensityCal[7][32];
    extern float aIntensityCal_old[1600][32];
    extern bool Curvesis_new;
    extern int g_ChannelNum[32][51];
    extern float CurvesRate[32];

    extern float temper;
    extern int tempPacketNum;
    extern int numOfLasers;
    extern int TEMPERATURE_RANGE;
