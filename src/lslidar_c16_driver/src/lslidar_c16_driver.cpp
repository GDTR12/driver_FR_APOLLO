...
namespace lslidar_c16_driver {
    ...
    lslidarDriver::lslidarDriver(rclcpp::Node::SharedPtr node, rclcpp::Node::SharedPtr private_nh) :  switch_status(0){
        scan_fill = false;
        ...
        lslidar_control = node->create_service<lslidar_c16_msg::srv::LslidarC16Control>(
            "lslidarcontrol", std::bind(&lslidarDriver::lslidarC16Control, this, std::placeholders::_1, std::placeholders::_2));
        // difop_output_ = node.advertise<lslidar_c16_msg::msg::LslidarC16Packet>(output_difop_topic, 10);
 ...
 */
    bool lslidarDriver::poll(void) {  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
        lslidar_c16_msg::msg::LslidarC16ScanUnified::Ptr scan(new lslidar_c16_msg::msg::LslidarC16ScanUnified);
...
//    }
        ...
        for (int i = 1; i < config_.npackets; ++i) {
            ...
            while (true) {
                ...
                //RCLCPP_DEBUG(rclcpp::get_logger("lslidarDriver"), "time_offset: " << config_.time_offset);
            ...
            if ((azi1 > 35000 && azi2 < 1000) || (azi1 < 500 && i > config_.npackets / 2)) {
                ...
                // RCLCPP_DEBUG(rclcpp::get_logger("lslidarDriver"), "azi1: " << azi1 <<"  "<< "azi2: " << azi2 << "   i:" << i);
                break;
            }
        }
        ...
        if (time_synchronization_) {
            ...
            lslidar_c16_msg::msg::LslidarC16Packet pkt = scan->packets[0];
    ...
    void lslidarDriver::difopPoll(void) {
        ...
        while (rclcpp::ok()) {
            // keep reading
            lslidar_c16_msg::msg::LslidarC16Packet difop_packet_msg;
            ...
            if (rc == 0) {
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("lslidarDriver"), "Publishing a difop data.");
                ...
                //RCLCPP_DEBUG(rclcpp::get_logger("lslidarDriver"), "time: " << difop_packet_msg.data[57]);
                //RCLCPP_DEBUG(rclcpp::get_logger("lslidarDriver"), "time: " << difop_packet_msg.data[56]);
    ...
    bool lslidarDriver::lslidarC16Control(lslidar_c16_msg::srv::LslidarC16Control::Request::SharedPtr req,
                                          lslidar_c16_msg::srv::LslidarC16Control::Response::SharedPtr res) {
        RCLCPP_WARN(rclcpp::get_logger("lslidarDriver"), "--------------------------");
        // sleep(1);
        lslidar_c16_msg::msg::LslidarC16Packet packet0;
        packet0.data[0] = 0x00;
        packet0.data[1] = 0x00;
        int rc_msop = -1;
        ...
        if (req->laser_control == 1) {
            ...
            RCLCPP_WARN(rclcpp::get_logger("lslidarDriver"), "receive cmd: %d,power on", req->laser_control);
            ...
            RCLCPP_WARN(rclcpp::get_logger("lslidarDriver"), "receive cmd: %d,power off", req->laser_control);
